  ...
  return (
    <section id="our-work" className="mx-auto max-w-7xl px-4 py-8 sm:py-10 lg:py-12 sm:px-6 lg:px-8">
      <div className="mb-8 sm:mb-12 text-center px-2">
        <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight text-white">Our Work</h2>
      </div>
      ...
      <div className="mb-6 sm:mb-8">
        ...
        >
          {['reel', 'podcast', 'longform', 'courses'].map((filter) => (
            <button
              key={filter}
              ...
              } group relative whitespace-nowrap px-4 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm font-bold shrink-0 transition-all hover:scale-105 focus:outline-none`}
              style={{ borderRadius: '9999px' }}
            >
              {filter === 'reel' ? 'Reels' : filter === 'podcast' ? 'Podcast' : filter === 'longform' ? 'Long Form' : 'Courses'}
            </button>
          ))}
        </nav>
      </div>
